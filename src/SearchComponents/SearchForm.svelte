<!--This represents a search form. It contains SearchFields for the properties to filter for, as well as button -->
<!--to fire the search.-->
<script>
    ...
    /** Stores the SearchFields to access their functions. */
    let searchFields = [];

    /** Contains the output for the viewer components */
    let requestInfos;

    /** The json containing the information to build the SearchForm */
    export let jsonFile;

    /** Contains the property name if a property filter has been predefined (before reaching this component) */
    export let predefProp;

    /** Contains the value of the predefined property filter */
     ...
     *
     * @returns {Promise<string>} the gravsearch query
     */
     ...
     *
     * @returns {string} The string to be added to the query.
     */
    ...
    /** TODO: catch error if no jsonFile was provided. */
    let resourceLabelPromise = getLabelForResource(jsonFile['ResName']);
</script>
...
{#each jsonFile['Props'] as prop} <!-- Create a SearchField for each prop in the json -->
    <SearchField bind:this={searchFields[searchFields.length]} prop={prop} {predefProp} {predefVal} parent="?mainres"/>
{/each}
